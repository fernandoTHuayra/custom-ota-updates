...
async function publish() {
  ...
  // 2. Process updates for each platform (exporting one-by-one to reduce memory usage)
  const requestedPlatforms = (process.env.EXPO_PUBLISH_PLATFORMS || 'ios,android')
    .split(',')
    .map((value) => value.trim().toLowerCase())
    .filter((value) => value === 'ios' || value === 'android');
  const platforms = requestedPlatforms.length > 0 ? requestedPlatforms : ['ios', 'android'];
  for (const platform of platforms) {
    console.log(`Running expo export for ${platform} in client...`);
    ...
    if (!fs.existsSync(metadataPath)) {
      console.error(
        `Error: metadata.json not found after exporting ${platform}. Did expo export fail?`,
      );
      process.exit(1);
    }
    ...
    const updateId = convertSHA256HashToUUID(
